...
func demoFlow(ctx context.Context) error {
	...
	// The same results screen the wizard ends on, so the demo teaches the
	// ending a real run has rather than a different one.
	showResults(report.DefaultDir, report.Redact(demo.TargetDSN()))
	return nil
}
